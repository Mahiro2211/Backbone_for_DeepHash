...
class Dilateformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dim=96,
                 ...
                 cpe_per_satge=False, cpe_per_block=True,bit=None):
        super().__init__()
...
@register_model
def dilateformer_small(bit,pretrained=True, **kwargs):
    model = Dilateformer(depths=[3, 5, 8, 3], embed_dim=72, num_heads=[ 3, 6, 12, 24 ],bit=bit,  **kwargs)
    model.default_cfg = _cfg()
    return model
