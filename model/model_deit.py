...
from utils.cf_vit_utils import batch_index_select, get_index
import numpy as np
import math
...
class MultiResoPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size_list=[112, 224], patch_size=16, in_chans=3, embed_dim=768):
        ...
        num_patches_list = [(img_size[1] // patch_size[1]) * (img_size[0] // patch_size[0]) for img_size in
                            img_size_list]
...
class CFVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size_list=[112, 224], patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.reuse_block = nn.Sequential(
            ...
            Mlp(in_features=embed_dim, hidden_features=mlp_ratio * embed_dim, out_features=embed_dim, act_layer=nn.GELU,
                drop=drop_rate)
        )
