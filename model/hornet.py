...
class HorNet(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000,
                 ...
                 gnconv=gnconv, block=Block, uniform_init=False,bit=None, **kwargs
                 ):
        super().__init__()
        dims = [base_dim, base_dim * 2, base_dim * 4, base_dim * 8]
        self.hash_layer = HashLayer(hash_bit=bit)
...
@register_model
def hornet_small_gf(pretrained=False, in_22k=False,bit=None, **kwargs):
    s = 1.0 / 3.0
    model = HorNet(depths=[2, 3, 18, 2], base_dim=96, block=Block,
                   gnconv=[
                       partial(gnconv, order=2, s=s),
...
@register_model
def hornet_base_gf(pretrained=False, in_22k=False,bit=None, **kwargs):
    s = 1.0 / 3.0
    model = HorNet(depths=[2, 3, 18, 2], base_dim=128, block=Block,
                   gnconv=[
                       partial(gnconv, order=2, s=s),
