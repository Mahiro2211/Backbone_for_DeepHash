...
def get_jaccard_similarity(w, scores, k, eps):
    ...
    Nk_mask_sum = Nk_mask.sum(1, keepdims=True)
    Nk_mask_sum = torch.where(Nk_mask_sum == 0, epsilon, Nk_mask_sum)
    intersection = (Nk_mask @ Nk_mask.T) / Nk_mask_sum.detach()
    Nk_mask_not = 1. - Nk_mask


    Nk_mask_not_sum = Nk_mask_not.sum(1, keepdims=True)

    Nk_mask_not_sum = torch.where(Nk_mask_not_sum == 0 , epsilon , Nk_mask_not_sum)

    # intersection_not = (Nk_mask_not @ Nk_mask_not.T) / Nk_mask_not.sum(1, keepdim=True).detach()
    intersection_not = (Nk_mask_not @ Nk_mask_not.T) / Nk_mask_not_sum.detach()
